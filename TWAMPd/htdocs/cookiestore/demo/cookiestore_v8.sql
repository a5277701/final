...
CREATE TABLE `order_content` (
  ...
  `quantity` int(2) unsigned NOT NULL COMMENT '訂購數量',
  PRIMARY KEY (`order_no`,`product_no`),
  KEY `product_no` (`product_no`),
  CONSTRAINT `order_content_ibfk_2` FOREIGN KEY (`product_no`) REFERENCES `product` (`product_no`) ON DELETE NO ACTION ON UPDATE CASCADE,
